...
function CustomLinks() {
  ...
  const {cannotPass, setCannotPass} = useCannotPassContext()
  const [cannotAdd, setCannotAdd] = useState<boolean>(false)
  const {previeuwAllowed} = usePrevieuwAllowedContext()
  ...
  <>
      ...
      <div data-testid="customLinks" className='customLinks'>
        ...
        <div data-testid="addNewLinkButton" className="addNewLinkButton" onClick={() => {
          ...
          links.length <= 4 ? addLink() : linkLimit()
        }}>
            <p>+ Add new link</p>
        </div>
        <div className={`linksContainer ${links.length > 1 && "maxHeight"}`}>
          ...
          :
          links.map((link: LinkType, index) => <Link key={index} index={index} /> )} 
        </div>
        <div className="linksButton">
            <div onClick={() => {
              links.length === 0 && setCannotPass(true)
              setTimeout(() => {
                setCannotPass(false)
              }, 1000)
              links.length !== 0 && setPart("profilDetails")
            }}>Save</div>
        </div>
      </div>

      <div className={`cannotPass ${
        cannotPass ? "cannotPassActive":"cannotPassDisable"
      }`}>
        <p>You have to create at least 1 link</p>
      </div>

      <div className={`cannotAdd ${
        cannotAdd ? "cannotAddActive":"cannotAddDisable"
      }`}>
        <p>You cannot create more than 5 links</p>
      </div>

      <div className={`cannotPrevieuw ${
        previeuwAllowed ? "cannotPrevieuwActive":"cannotPrevieuwDisable"
      }`}>
